...
//méthode qui récupère le detail d'un artiste 
export const fetchArtistDetail = (id) => async (dispatch) => {
    try {
        dispatch(setLoading(true))
        const response = await axios.get(`${API_URL}/artists?id=${id}&albums.isActive=true`)
        dispatch(setArtistDetail(response.data))
    } catch (error) {
        console.log(`Erreur lors de la requete du detail d'un artiste : ${error}`)
    } finally {
